...
// import { Target, Clock, AlertTriangle, TrendingUp } from '@heroicons/react/24/outline';
    ...
    'AX': {
      ...
      recommendations: [
        <span title="Обеспечить постоянное наличие на складе означает поддерживать достаточный запас товара, чтобы всегда удовлетворять спрос.">Обеспечить постоянное наличие на складе</span>,
        ...
        <span title="Развивать долгосрочные отношения с поставщиками - это строить крепкие, доверительные связи с поставщиками для обеспечения стабильных и выгодных поставок.">Развивать долгосрочные отношения с поставщиками</span>
      ],
      risks: [
        ...
        <span title="Появление конкурентов с лучшим предложением - это риск того, что новые или существующие конкуренты предложат более привлекательные товары или условия.">Появление конкурентов с лучшим предложением</span>
      ],
      kpis: [
    ...
    'AY': {
      ...
      reasons: [
        <span title="Высокая доля в выручке (группа A) — товар приносит значительный доход, но его продажи сильно зависят от сезона (например, новогодние подарки, летняя одежда).">Высокая доля в выручке, но сезонный характер спроса</span>,
        ...
        <span title="Требует точного прогнозирования и планирования — для минимизации рисков переизбытка или дефицита необходимо очень точно предсказывать спрос и планировать закупки.">Требует точного прогнозирования и планирования</span>
      ],
      recommendations: [
        <span title="Создать детальный план сезонных закупок — разработать чёткий график закупок и поставок, ориентированный на пики и спады сезонного спроса.">Создать детальный план сезонных закупок</span>,
        ...
        <span title="Разработать маркетинговые кампании под сезоны — проводить рекламные акции и мероприятия, синхронизированные с началом и окончанием сезона продаж.">Разработать маркетинговые кампании под сезоны</span>,
        <span title="Подготовить альтернативные каналы сбыта — искать другие способы реализации товара в межсезонье, чтобы избежать затоваривания (например, оптовые продажи, другие рынки).">Подготовить альтернативные каналы сбыта</span>
      ],
      risks: [
    ...
    'AZ': {
      ...
      reasons: [
        <span title="Высокая доля в выручке означает, что товар приносит значительный доход, но его нерегулярный спрос делает продажи непредсказуемыми.">Высокая доля в выручке, но нерегулярный спрос</span>,
      ...
      recommendations: [
        <span title="Провести глубокий анализ причин нестабильности — необходимо выяснить основные факторы, вызывающие нерегулярность спроса (например, внешние тренды, действия конкурентов, внутренние ошибки).">Провести глубокий анализ причин нестабильности</span>,
        <span title="Изучить поведение клиентов и факторы спроса — понять, кто, когда и почему покупает товар, а также какие внешние и внутренние факторы влияют на его продажи.">Изучить поведение клиентов и факторы спроса</span>,
        <span title="Рассмотреть сегментацию клиентской базы — разделить клиентов на группы по их покупательскому поведению, чтобы разработать более целенаправленные стратегии.">Рассмотреть сегментацию клиентской базы</span>,
        <span title="Разработать гибкую систему поставок — создать систему, которая позволяет быстро адаптироваться к изменениям спроса, минимизируя риски излишков или нехватки.">Разработать гибкую систему поставок</span>,
        revenue > 60000 ? <span title="Создать отдельную команду для управления товаром — выделить специальную группу специалистов, которая будет заниматься исключительно этим товаром из-за его критической важности и сложности.">Создать отдельную команду для управления товаром</span> :
        <span title="Назначить ответственного менеджера — закрепить за этим товаром конкретного сотрудника, отвечающего за его показатели и управление.">Назначить ответственного менеджера</span>
      ],
      risks: [
    ...
    'BX': {
      ...
      reasons: [
        <span title="Средняя доля в выручке со стабильным спросом — товар приносит умеренный, но постоянный доход.">Средняя доля в выручке со стабильным спросом</span>,
      ...
      recommendations: [
        ...
        <span title="Рассмотреть возможности роста продаж — искать новые каналы сбыта или маркетинговые стратегии для увеличения объемов продаж этого товара.">Рассмотреть возможности роста продаж</span>
      ],
      risks: [
        ...
        <span title="Появление более эффективных аналогов — риск того, что конкуренты выпустят аналогичные товары, которые будут превосходить ваш по цене или качеству.">Появление более эффективных аналогов</span>,
    ...
    'BY': {
      ...
      reasons: [
        <span title="Средняя прибыльность с сезонными колебаниями — товар приносит умеренный доход, но его продажи сильно зависят от сезона.">Средняя прибыльность с сезонными колебаниями</span>,
        <span title="Требует планирования под сезонные пики — для эффективного управления необходимо заранее планировать закупки и запасы к периодам высокого спроса.">Требует планирования под сезонные пики</span>,
      ...
      recommendations: [
        <span title="Создать сезонные модели планирования — разработать специальные модели прогнозирования и планирования, учитывающие сезонность спроса.">Создать сезонные модели планирования</span>,
        ...
        <span title="Развивать межсезонные продажи — искать способы стимулирования спроса на товар вне его основного сезона (например, скидки, новые применения товара).">Развивать межсезонные продажи</span>,
        <span title="Искать новые рынки сбыта — изучать возможности продажи товара на других рынках или в других регионах, где сезонность может отличаться.">Искать новые рынки сбыта</span>
      ],
      risks: [
    ...
    'BZ': {
      ...
      reasons: [
        <span title="Средняя прибыльность с высокой нестабильностью — товар приносит умеренный доход, но его продажи очень непредсказуемы.">Средняя прибыльность с высокой нестабильностью</span>,
        ...
        <span title="Требует особого подхода к управлению — из-за непредсказуемости, для этого товара нужны специальные стратегии контроля и планирования.">Требует особого подхода к управлению</span>
      ],
      recommendations: [
        ...
        <span title="Использовать систему 'точно в срок' (Just-In-Time) — заказывать и получать товар только тогда, когда он нужен для продажи, без создания больших запасов.">Использовать систему "точно в срок"</span>,
        <span title="Развивать быстрые каналы поставок — иметь возможность получать товар очень быстро в случае внезапного спроса.">Развивать быстрые каналы поставок</span>,
        <span title="Рассмотреть возможность отказа от товара — если риски и затраты на управление слишком высоки, стоит подумать о прекращении продажи этого товара.">Рассмотреть возможность отказа от товара</span>
      ],
      risks: [
    ...
    'CX': {
      ...
      recommendations: [
        ...
        <span title="Рассмотреть аутсорсинг — передать часть или все функции по управлению этим товаром сторонней компании (например, логистическому оператору).">Рассмотреть аутсорсинг</span>,
      ...
      risks: [
        ...
        <span title="Возможные сбои в автоматизации — риск ошибок или отказов в автоматизированных системах, что может привести к проблемам с поставками или запасами.">Возможные сбои в автоматизации</span>,
    ...
    'CY': {
      ...
      reasons: [
        <span title="Низкая прибыльность с сезонными колебаниями — товар приносит небольшой доход, и его продажи сильно зависят от сезона.">Низкая прибыльность с сезонными колебаниями</span>,
        ...
        <span title="Требует минимальных инвестиций — нет смысла вкладывать много денег в продвижение или запасы этого товара.">Требует минимальных инвестиций</span>
      ],
      recommendations: [
        ...
        <span title="Рассмотреть работу с дропшиппингом — продавать товар без собственного склада, отправляя заказы напрямую поставщику для доставки клиенту.">Рассмотреть работу с дропшиппингом</span>,
        <span title="Оценить целесообразность продолжения продаж — проанализировать, стоит ли вообще продолжать продавать этот товар, если он приносит мало пользы и много проблем.">Оценить целесообразность продолжения продаж</span>
      ],
      risks: [
    ...
    'CZ': {
      ...
      reasons: [
        <span title="Низкая прибыльность и нестабильный спрос — товар приносит мало денег и его продажи непредсказуемы, что делает его невыгодным.">Низкая прибыльность и нестабильный спрос</span>,
      ...
      risks: [
        ...
        <span title="Возможные контрактные обязательства — могут быть контракты с поставщиками или клиентами, которые усложняют быстрый вывод товара.">Возможные контрактные обязательства</span>
      ],
      kpis: [
